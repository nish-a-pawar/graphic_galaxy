...
const team = [
  { name: 'Vaibhav Biradar', role: 'Founder & Senior Designer', initials: 'VB', color: 'from-amber-400 to-amber-600'   },
  { name: 'Nisha Pawar',     role: 'General Manager',           initials: 'NP', color: 'from-purple-400 to-purple-600' },
  { name: 'Tanmay Patil',    role: 'Senior Graphic Designer',   initials: 'TP', color: 'from-blue-400 to-blue-600'     },
  { name: 'Shraddha Dongre', role: 'Junior Graphic Designer',   initials: 'SD', color: 'from-pink-400 to-pink-600'     },
  { name: 'Prajwal Aawti',   role: 'Design Intern',             initials: 'PA', color: 'from-teal-400 to-teal-600'     },
];
          ...
          <div ref={teamRef} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {team.map((m, i) => (
              <div key={i} className="team-card bg-[#0B0F14] border border-[#2D3748] rounded-3xl p-6 flex flex-col items-center text-center hover:border-amber-400/50 hover:shadow-[0_0_20px_rgba(245,158,11,0.1)] transition-all duration-300 hover:-translate-y-1" style={{ opacity: 0 }}>
                <div className={`w-16 h-16 rounded-2xl bg-linear-to-br ${m.color} flex items-center justify-center text-[#0B0F14] font-black text-lg mb-4`}>
                  {m.initials}
                </div>
